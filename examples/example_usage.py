...
from vlpim.immunogenicity_optimization_pipeline import ImmunogenicityOptimizer, PipelineConfig, ImmunogenicityMode
...
def example_advanced_usage():
    ...
    config = PipelineConfig(
        ...
        max_candidates=20,
        rmsd_threshold=1.5,
        dg_dsasa_threshold=-0.3,
        buns_threshold=3,
        packstat_threshold=0.7
    )
...
def example_custom_scoring():
    ...
    config = PipelineConfig(
        ...
        dg_dsasa_threshold=-0.3,  # Stricter dG/dSASA threshold
        buns_threshold=3,  # Stricter BUNS threshold
        packstat_threshold=0.7  # Stricter packstat threshold
    )
    ...
    try:
        ...
        print(f"Using dG/dSASA threshold: {config.dg_dsasa_threshold}")
        print(f"Using BUNS threshold: {config.buns_threshold}")
        print(f"Using packstat threshold: {config.packstat_threshold}")
